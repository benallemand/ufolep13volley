...
class Rank extends Generic
{
     ...
     * @throws Exception
     */
    public function get_rank_for_cup($code_competition)
    {
        $sql = "SELECT @r := @r + 1 AS rang,
                ...
                FROM (SELECT e.id_equipe,
                             ...
                             SUM(CASE
                                     WHEN m.id_equipe_dom = e.id_equipe
                                         THEN m.set_1_dom + m.set_2_dom + m.set_3_dom + m.set_4_dom + m.set_5_dom
                                     ELSE m.set_1_ext + m.set_2_ext + m.set_3_ext + m.set_4_ext + m.set_5_ext END)                                                                                         AS points_pour,
                             SUM(CASE
                                     WHEN m.id_equipe_dom = e.id_equipe
                                         THEN m.set_1_ext + m.set_2_ext + m.set_3_ext + m.set_4_ext + m.set_5_ext
                                     ELSE m.set_1_dom + m.set_2_dom + m.set_3_dom + m.set_4_dom + m.set_5_dom END)                                                                                         AS points_contre,
                             SUM(CASE
                                     WHEN m.id_equipe_dom = e.id_equipe
                                         THEN m.set_1_dom + m.set_2_dom + m.set_3_dom + m.set_4_dom + m.set_5_dom
                                     ELSE m.set_1_ext + m.set_2_ext + m.set_3_ext + m.set_4_ext + m.set_5_ext END) - SUM(CASE
                                                                                                                             WHEN m.id_equipe_dom = e.id_equipe
                                                                                                                                 THEN m.set_1_ext + m.set_2_ext + m.set_3_ext + m.set_4_ext + m.set_5_ext
                                                                                                                             ELSE m.set_1_dom + m.set_2_dom + m.set_3_dom + m.set_4_dom + m.set_5_dom END) AS diff_points,
                             c.penalite                                                                                                                                                                    AS penalites,
                             SUM(IF(e.id_equipe = m.id_equipe_dom AND m.forfait_dom = 1, 1, 0)) +
                             SUM(IF(e.id_equipe = m.id_equipe_ext AND m.forfait_ext = 1, 1, 0))                                                                                                            AS matches_lost_by_forfeit_count,
                             c.report_count
                      ...
                      ORDER BY points DESC, diff_sets DESC, diff_points DESC, c.rank_start) z,
                     (SELECT @r := 0) y";
        $bindings = array();
