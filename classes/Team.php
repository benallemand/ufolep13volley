...
class Team extends Generic
{
    ...
    public function getSql($query = "1=1"): string
    {
        ...
        GROUP_CONCAT(DISTINCT CONCAT(CONCAT(g.ville, ' - ', g.nom, ' - ', g.adresse, ' - ', g.gps), ' (',cr.jour, ' à ', cr.heure,')', IF(cr.has_time_constraint > 0, ' (CONTRAINTE HORAIRE FORTE)', ''), IFNULL(CONCAT(' [', g.remarques, ']'), '')) SEPARATOR ', ') AS gymnasiums_list,
        e.web_site,
        e.id_photo,
        p.path_photo,
        e.is_cup_registered,
    ...
    public function getQuickDetails($idEquipe)
    {
        $sql = "SELECT 
            ...
            jsupp.telephone AS telephone_2,
            jresp.email,
            GROUP_CONCAT(DISTINCT CONCAT(CONCAT(g.ville, ' - ', g.nom, ' - ', g.adresse, ' - ', g.gps), ' (',cr.jour, ' à ', cr.heure,')', IF(cr.has_time_constraint > 0, ' (CONTRAINTE HORAIRE FORTE)', ''), IFNULL(CONCAT(' [', g.remarques, ']'), '')) SEPARATOR '\n') AS gymnasiums_list,
            e.web_site,
            p.path_photo
        FROM equipes e 
        LEFT JOIN classements cl ON cl.id_equipe = e.id_equipe
    ...
    public function getActiveTeams()
    {
        ...
        GROUP BY cl.code_competition, 
                 ...
                 e.web_site, 
                 p.path_photo
        ORDER BY comp.libelle, cl.division, c.nom, nom_equipe";
        return $this->sql_manager->execute($sql);
    }
