...
function deleteBlacklistTeam($ids)
{
    global $db;
    conn_db();
    $sql = "DELETE FROM blacklist_team WHERE id IN($ids)";
    $req = mysqli_query($db, $sql);
    disconn_db();
    if ($req === FALSE) {
        return false;
...
function saveTimeSlot()
{
    ...
    $inputs = array(
        ...
        'usage_priority' => filter_input(INPUT_POST, 'usage_priority')
    );
    conn_db();
    if (empty($inputs['id'])) {
        $sql = "INSERT INTO";
