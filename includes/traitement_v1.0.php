...
if (isset($_GET['a'])&&!empty($_GET['a'])) {$action = $_GET['a'];}

if ($action=="am")	{ajout_match();}
if ($action=="an")	{ajout_news(); }
if ($action=="cm")	{certif_match();}
if ($action=="gpa")	{penalite("a");}
if ($action=="gpe")	{penalite("e");}
if ($action=="ie")	{inscrit_equipe();}
if ($action=="me")	{maj_equipe();}
if ($action=="mn")	{maj_news();}
if ($action=="mr")	{maj_result();}
if ($action=="se")	{suppr_equipe();}
if ($action=="sec")	{suppr_equipe_compet();}
if ($action=="sm")	{suppr_match();}
if ($action=="sn")	{suppr_news();}

elseif ($action=="admin_team") { admin_team(); }
elseif ($action=="admin_journee") { admin_journee(); }
elseif ($action=="auth") { auth(); }
elseif ($action=="maj_team") { maj_team();}
elseif ($action=="deconn") { deconn();}
elseif ($action=="add_team") { add_team();}
elseif ($action=="maj_certif") { maj_certif();}
...
if ((empty($_POST['login']))||(empty($_POST['password'])) ) 
  ...
  {mysql_close(); die('<META HTTP-equiv="refresh" content=0;URL=../portail.php?log=noauth>');}
...
if (mysql_num_rows($req) > 0)			// si la requête comporte un résultat dans la table compte_acces
  ...
  { $data = mysql_fetch_assoc($req);	
	...
	if ($data['password'] != $password) {mysql_close(); die('<META HTTP-equiv="refresh" content=0;URL=../portail.php?log=wpass');}
...

	  {
        ...
        if($id_equipe=="999") {$_SESSION['id_equipe']="admin"; die('<META HTTP-equiv="refresh" content=0;URL=../index.php>');}  

	    else { $_SESSION['id_equipe'] = $id_equipe; die('<META HTTP-equiv="refresh" content=0;URL=../portail.php>');}						// Sinon c'est une équipe
...
$score_equipe_dom = $_POST['score_equipe_dom'];		$score_equipe_ext = $_POST['score_equipe_ext'];;
$set_1_dom = $_POST['set_1_dom'];	$set_2_dom = $_POST['set_2_dom'];	
$set_3_dom = $_POST['set_3_dom'];	$set_4_dom = $_POST['set_4_dom'];
$set_5_dom = $_POST['set_5_dom'];	if(empty($gagnea5_dom)) {$gagnea5_dom=0;} else {$gagnea5_dom = $_POST['gagnea5_dom'];}
$set_1_ext = $_POST['set_1_ext'];	$set_2_ext = $_POST['set_2_ext'];	
$set_3_ext = $_POST['set_3_ext'];	$set_4_ext = $_POST['set_4_ext'];
$set_5_ext = $_POST['set_5_ext'];	if(empty($gagnea5_ext)) {$gagnea5_ext=0;} else {$gagnea5_ext = $_POST['gagnea5_ext'];}
$code_match = $_POST['code_match']; $compet = $_POST['compet']; $division = $_POST['division'];
$heure_reception = $_POST['heure_reception']; $date_reception = date_uk($_POST['date_reception']);

// Traitement des matches gagnés à 5 ==============================================================================
if(empty($gagnea5_dom)) {$gagnea5_dom=0;}
if(empty($gagnea5_ext)) {$gagnea5_ext=0;}

//Traitement des matches forfait ==================================================================================
$total_sets_dom = $set_1_dom+$set_2_dom+$set_3_dom;
$total_sets_ext = $set_1_ext+$set_2_ext+$set_3_ext;
if($total_sets_dom==0&&$total_sets_ext==75)	{ $forfait_dom = 1; } // Si le match est perdu par forfait pour l'équipe dom
else { $forfait_dom = 0; }
if($total_sets_dom==75&&$total_sets_ext==0)	{ $forfait_ext = 1; } // Si le match est perdu par forfait pour l'équipe ext
else { $forfait_ext = 0; }

//Traitement des reports
if(isset($_POST['date_originale'])&&!empty($_POST['date_originale']))
  {
  if($_POST['date_originale']!=$_POST['date_reception']) {$report = 1;}
  else {$report = 0;}
  }
...
$req = mysql_query($sql) or die('Erreur SQL !<br>'.$sql.'<br>'.mysql_error());  
if (mysql_num_rows($req)==1) {$data = mysql_fetch_assoc($req); $penalite = $data['penalite']; $division = $data['division'];}
if ($action=="a") {$penalite++;}
elseif ($action=="e") {$penalite--;}

// Si $penalite est négatif on le positionne à 0
if($penalite<0) {$penalite=0;}
